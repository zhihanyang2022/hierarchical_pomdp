import torch
import numpy as np
from DDPG_reward_only import DDPG
...
class HAC:
    def __init__(self, k_level, H, state_dim, action_dim, render, threshold, 
        ...
        for l in range(k_level-1):
            for_top_level = (l==k_level-2)
            self.HAC.append(DDPG(state_dim, state_dim, state_bounds, state_offset, lr, H, for_top_level=for_top_level))
            self.replay_buffer.append(ReplayBuffer(for_top_level=for_top_level))
    ...
    def check_goal(self, state, goal, threshold, for_top_level=False):
        if for_top_level:

            # here, goal would be just 1

            true_goal = np.array([0.48, 0.04])
            true_goal_achieved = True
            true_threshold = np.array([0.01, 0.02])

            for i in range(self.state_dim):
                if abs(state[i]-true_goal[i]) > true_threshold[i]:
                    true_goal_achieved = False

            return true_goal_achieved
    ...
    def run_HAC(self, env, i_level, top_level, state, goal, is_subgoal_test):
        ...
        for _ in range(self.H):
            ...
            if (i_level == top_level):
                action = self.HAC[i_level].select_action(state)  # the top policy has one goal, so no need to be goal conditioned
            else:
            ...
            if i_level > 0:
                ...
                next_state, done = self.run_HAC(env, i_level-1, top_level, state, action, is_next_subgoal_test)
                
                # if subgoal was tested but not achieved, add subgoal testing transition
                if is_next_subgoal_test and not self.check_goal(action, next_state, self.threshold, for_top_level=(i_level==top_level)):
                    if (i_level == top_level):
                        self.replay_buffer[i_level].add((state, action, -self.H, next_state, 0.0, float(done)))
                    else:
            ...
            if (i_level == top_level):
                ...
                goal_achieved = self.check_goal(next_state, None, None, True)

                # hindsight action transition
                if goal_achieved:
                    self.replay_buffer[i_level].add((state, action, 0.0, next_state, 0.0, float(done)))
                else:
                    self.replay_buffer[i_level].add((state, action, -1.0, next_state, self.gamma, float(done)))
