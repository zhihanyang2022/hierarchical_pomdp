...
import gym
import numpy as np
from HAC_reward_only import HAC

device = torch.device("cuda:0" if torch.cuda.is_available() else "cpu")

def train():
    #################### Hyperparameters ####################
    # env_name = "gym_visual:continuous-mountain-car-v0"
    ...
    goal_state = np.array([1.0], dtype=float)       # final goal state to be achived
    true_goal_state = np.array([0.48, 0.04])
    threshold = np.array([0.01, 0.02])  # threshold value to check if goal state is achieved
    
    # HAC parameters:
    k_level = 4                 # num of levels in hierarchy
    ...
    for i_episode in range(1, max_episodes+1):
        ...
        last_state, done = agent.run_HAC(env, k_level-1, k_level-1, state, goal_state, False)
        
        if agent.check_goal(last_state, true_goal_state, threshold):
            print("################ Solved! ################ ")
